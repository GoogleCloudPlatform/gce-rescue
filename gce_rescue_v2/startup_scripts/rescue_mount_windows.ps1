...
# with the same-image-family rescue disk) - their BCD is realigned below.
$guidChangedDisks = @()
...
foreach ($disk in $disks) {
    ...
    try {
        ...
        # disk online makes Windows detect the signature collision and
        # REGENERATE the affected disk's GUID, invalidating the device
        # references in that disk's own BCD - after restore the firmware halts
        # at 0xc000000e/0xc0000225. Restoring the old GUID here does NOT work
        # (live-tested): the rescue disk with the same GUID is still online, so
        # the collision immediately re-exists and Windows re-resolves it later.
        # Instead we DETECT the change and realign the affected disk's BCD to
        # its new, now-stable identity with bcdboot after its volumes mount
        # (see the BCD realignment section below).
        $originalGuid = $null
        try {
        ...
        if ($originalGuid) {
            try {
                ...
                if ($currentGuid -ne $originalGuid) {
                    Write-Log "  Disk GUID was regenerated by Windows ($originalGuid -> $currentGuid) - BCD realignment needed (issue #126)"
...
# BCD realignment for GUID-regenerated disks (issue #126).
# Windows resolved a signature collision by giving the affected disk a NEW
# GPT GUID; that new identity is stable (no collision remains), but the
# disk's BCD still references the OLD identity, so it cannot boot after
# restore. bcdboot rebuilds the BCD (and boot files) from the offline
# \Windows against the disk's CURRENT identity - the standard, supported
# offline repair. Runs ONLY for disks whose GUID actually changed: a
# collision-free rescue never touches the BCD. The pre-rescue snapshot is
# the rollback.
foreach ($diskNumber in $guidChangedDisks) {
    Write-Log ""
    Write-Log "=== BCD Realignment (issue #126) for Disk $diskNumber ==="
    try {
        # Find the offline Windows installation on THIS disk via its mounted
        # partitions (drive letters were assigned above).
        $winLetter = $null
        foreach ($part in Get-Partition -DiskNumber $diskNumber -ErrorAction SilentlyContinue) {
            if ($part.DriveLetter -and
                (Test-Path "$($part.DriveLetter):\Windows\System32\config\SYSTEM")) {
                $winLetter = $part.DriveLetter
                break
            }
        }
        if (-not $winLetter) {
            Write-Log "  No offline Windows installation found on disk $diskNumber - skipping BCD realignment"
            continue
        }
        Write-Log "  Offline Windows found at ${winLetter}:\Windows"

        # Locate this disk's EFI System Partition and give it a temporary
        # letter so bcdboot can write to it (UEFI). No ESP -> BIOS firmware.
                ...
                foreach ($letter in 'S', 'T', 'U', 'V', 'W') {
                    ...
                    if ($letter -notin $used) {
                        try {
                            ...
                            $sysLetter = $letter
                            $assignedEspLetter = $true
                            Write-Log "  Mounted EFI system partition at ${letter}:"
                        } catch {
        ...
        Write-Log "  Realigning BCD to the disk's new identity: $bcdCmd"
        $bcdOutput = & cmd.exe /c "$bcdCmd 2>&1"
        if ($LASTEXITCODE -eq 0) {
            Write-Log "  SUCCESS: BCD realigned for disk $diskNumber (issue #126 collision repaired)"
        } else {
            Write-Log "  ERROR: bcdboot failed (exit $LASTEXITCODE): $bcdOutput"
            Write-Log "  The VM may not boot after restore - repair the BCD manually before restoring"
        }

        # Release the temporary ESP letter so restore is clean.
        if ($assignedEspLetter -and $sysLetter) {
            try {
        ...
        Write-Log "  ERROR during BCD realignment: $_"
    }
}
